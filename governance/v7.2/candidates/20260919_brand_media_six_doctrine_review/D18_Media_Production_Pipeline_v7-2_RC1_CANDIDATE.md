# DCSE D18 | Media Production Pipeline v7.2 RC1 Candidate

**Document ID:** DCSE-D18-v7.2-RC1-CANDIDATE
**Date:** 2026-09-19
**Scope:** ALL authorized lanes and media formats, bounded by current D12/entity/profile and access
**Status:** CANDIDATE / NOT RATIFIED; DCS Level 0 controls promotion and public release
**Lineage:** Uploaded original D18, main v7.2 carried-forward D18, proposed brand-motion ident Section 9 in PR #158. Legacy model assignments and status labels in the inherited text are provenance, not authority or proof of runtime capability.
**Control:** D12 is media governance; D18 is a production methodology. D21 preflight and D22 evidence reconciliation remain required.

---
> **v7.2 consolidated-source projection (2026-09-11):** This file places the current carried-forward D18 subject matter inside the self-contained v7.2 governance package. Source lineage: `governance/v7.1/source/doctrines/D18_Media_Production_Pipeline.md`. Embedded legacy version labels, local file URLs, and pre-v7.2 routing references in the inherited body are provenance only and SHALL NOT control v7.2 runtime routing. Active authority, lane routing, source identity, and lifecycle state resolve through the operative v7.2 R5 controller, D21, D22, D05, the v7.2 Doctrine Index, and later DCS directives. This projection does not convert historical status text into a new promotion event.
...
---
## 10. v7.2 runtime execution profile and reconciled short-form production

### 10.1 Format-aware planning, not impossible universal requirements
At preflight, resolve `media_format` (6–8s brand ident / short social / long promo / education / narration / silent visual), `identity_version`, original binary access and rights, distribution channel, audience, source duration/resolution, tool capability, approver and exact exit evidence. Use the storyboard structure, voice and CTA **only where appropriate to that declared format**. A no-voice seven-second brand ident SHALL NOT be rejected merely because it has no 30–45s narrated education segment or a customer CTA. Model/maker choices are capability-driven and authorized by DCS; a legacy preference matrix is not authority or evidence of a connected renderer.

### 10.2 Model-output reality check
The pipeline SHALL distinguish `BRIEF_ONLY`, `PROMPT_READY`, `RENDER_REQUESTED`, `RENDER_RETURNED`, `EDITED_MASTER`, `QA_VALIDATED`, `DCS_APPROVED`, `RELEASED`. A generated still, project file, prompt, thumbnail or request receipt does **not** imply that a playable video exists. A delivery receipt SHALL include the actual binary's authorized path, source/final SHA-256, duration observed by a media probe, exact aspect ratio, resolution provenance (native versus upscaled), frame rate, mime/codec, audio/rights, reviewable preview and independent validation findings as required. Do not state 4K master is native 4K merely because the container reports 3840×2160.

### 10.3 Identity motion treatment
The precise source art remains the final-identity end plate. For CTJ Unified, the final DCS-designated 16:9 logo with **no top tagline** and a single **bottom `CLARITY • PERSPECTIVE • ACTION`** is the only source eligible to resolve the ident. First recover original binary, verify current source hash/version/rights and the exact family/product wordmark before selecting image-to-video or compositor paths. Animate narrative movement independently when a model warps text; hold original plate static for >=1.5s after the concept becomes legible. Unknown source asset is `ACCESS_PENDING`, not `VERIFIED_ABSENT`. The past right-panel comparison, old hero and transparency experiment remain lineage, not substitute masters.

### 10.4 QA, roles and feedback
The storyboard operator may create candidate motion, but cannot self-promote or invent a new brand name, release rights, or edited footage. The actual independent validator must test text invariants including ™, duration, last-frame exactness, accessibility reduced-motion/static alternate, sound license, legibility at intended screen sizes, absence of flash risk, orientation and source/derivative consistency. Store failure reason and timecode and revise only failing elements when rollback is safe. The maker-ready prompt and Gemini Gem instructions are operational aids and SHALL NOT be copied into a public media file as internal doctrine prose or substituted for final QA.

### 10.5 Challenge tests
`D18-001` Prompts marked as rendered MP4 -> FAIL. `D18-002` Seven-second ident forced to narrated longer structure -> FAIL. `D18-003` Logo text changes but video is rated brand-safe by visual similarity -> FAIL. `D18-004` No readback of exact source binary/hash -> ACCESS_PENDING, no final identity handoff. `D18-005` Muted/autoplay fails reduced-motion user setting -> accessibility gate fails. `D18-006` video file validates but no DCS use/release approval -> remains QA_VALIDATED, not RELEASED.

**Exit:** V7.2 RC1 CANDIDATE; D12 short-ident addendum/this ident profile remain PR #158 candidates pending D21/D05/D22 routing.
