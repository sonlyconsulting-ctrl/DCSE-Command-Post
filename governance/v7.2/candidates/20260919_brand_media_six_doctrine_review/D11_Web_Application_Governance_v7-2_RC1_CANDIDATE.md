# DCSE Doctrine D11: Web and Application Governance v7.2 RC1 Candidate

**Document ID:** DCSE-D11-v7.2-RC1-CANDIDATE  
...
**Candidate lineage:** v7.1 RC3, compared to operative carried-forward v7.2 D11_HTML_Wix_App; original v7.1 RC3 remains a candidate  
...
**Promotion effect:** NONE; no platform architecture, access, secrets, or public deployment is authorized by this candidate.  
...
---
## 22. v7.2 reconciliation: compatibility and supersession boundaries

This full RC1 is a **v7.2 candidate**, not an automatically operative rewrite of `governance/v7.2/doctrines/D11_HTML_Wix_App_v7-2.md`. The inherited HTML/Wix/App platform-specific requirements remain applicable when its verified platform adapter is in use; they SHALL NOT be imported into a different CTJ application solely because the doctrine filename mentions Wix. The v7.1 RC3 platform-neutral application model and test matrix are retained here for DCS review; independent verification and D05/D22 gates precede exact supersession.

### 22.1 Parent SC / CTJ identity contract
Before implementing a product page or media-bearing brand header, resolve the *current approved* D09 parent and child profiles. Preserve parent SC design-token inheritance and CTJ-specific, explicitly approved variations; never copy a light-page palette or payment provider from one SC subproduct to another by guesswork. The SC display typography carried in operative D11 is `Cormorant Garamond, Georgia, serif`; subheaders/labels `Barlow Condensed, Arial Narrow, sans-serif`; data `DM Mono, monospace`. These are inherited SC defaults pending approval of a new versioned D09 type profile; they are **not** universal fonts imposed on unrelated entities. Readable body/UI font must be declared and licensed/validated per product; `Inter` in existing source is a candidate, not proof of global ratification. The original raster CTJ logo's exact font remains unknown absent editable evidence.

A logo, icon, product hero, motion ident and thumbnail have different uses. A website SHALL reference the correct identity/source version and approved use; displaying an image file cannot promote that file as final brand artwork. Where the logo or hero is a raster, maintain original aspect without stretching; define safe areas and `object-fit`/responsive crop rules for wide, mobile and high-zoom layouts. The latest CTJ Unified logo is a distinct product asset; the old comparison graphic is not the final production end frame. No new “Executive Suite” formal product title by inference.

### 22.2 Media on websites and web apps
For brand-motion idents, use D12/D18 approved **6–8 s** assets only after retrieval/rights, duration/aspect, typography and source-logo-fidelity checks. Default in-page autoplay, if allowed, SHALL be muted, nonblocking, pausable or stop automatically, respect `prefers-reduced-motion`, provide a static accessible alternative, and never prevent navigation or introduce seizure-risk flashes. Avoid relying on video for text that must be read or understood. Captions/transcripts apply where speech or meaningful audio requires them; silent music-only idents do not need invented spoken captions. Track compressed media sizing and runtime costs against the actual page performance budget; 4K is a production master target, NOT the mandatory downloaded asset for every phone.

### 22.3 Interface/commerce evidence
A customer-facing payment CTA or access label must reflect a verified actual endpoint and entitlement contract, not a design mockup. For CTJ, do not silently route through Wix. Follow its separately verified checkout/payment-submission -> merchant-side verification -> entitlement/access sequence. No fabricated verified-customer quote, instant access, lifetime access, logo watermark, or customer-facing internal DDNA control without specific approved source and actual implementation. Release QA SHALL test links, responsive headers/footers, button action, error/retry, authentication/access controls, payments and static-video fallbacks *on target device and hosted runtime*, not infer functionality from a PNG or code snippet.

### 22.4 Explicit challenge tests
`D11-025` CTJ routed to Wix because D11 has “Wix” in legacy filename => route isolation fails. `D11-026` logo and icon swapped at favicon scale => brand-size QA fails. `D11-027` 4K brand video forced to mobile, no reduced-motion alternative => media/performance accessibility result fails. `D11-028` exact brand tokens available but wrong product profile or unknown font silently applied => profile resolution fails. `D11-029` storefront claims payment completed from user screenshot without merchant verification => commerce/access validation fails. `D11-030` candidate v7.2 document exists but no D05 promotion => operative-source pointer is unchanged.

**Exit:** V7.2 RC1 CANDIDATE; platform-neutral core and adapters await DCS decision, code/runtime impact review, independent validation and D22 reconciliation. Unaffected existing application work may continue under already verified applicable authority.
