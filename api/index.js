...
function loginPage(error = '', notice = '') {
  ...
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>DCSE Secure Access</title><style>${authStyles()}</style></head><body><main class="box"><div class="brand">SC / DCSE SECURE ACCESS</div><div class="sub">Authorized operator sign-in</div><div class="message err">${safeError}</div><div class="message ok">${safeNotice}</div><form method="post" action="/?dcse_auth=login"><input class="field" id="loginEmail" type="email" name="email" autocomplete="username" required placeholder="Email"><input class="field" type="password" name="password" autocomplete="current-password" required placeholder="Password"><button class="btn" type="submit">Sign In</button></form><button class="btn secondary" id="forgotPassword" type="button">Send password reset email</button><div class="message" id="resetStatus" role="status" aria-live="polite"></div></main><script src="${SUPABASE_BROWSER_SRC}" crossorigin="anonymous"></script><script nonce="${nonce}">(() => {const config=${config};const button=document.getElementById('forgotPassword');const status=document.getElementById('resetStatus');const emailInput=document.getElementById('loginEmail');if(!config.url||!config.key||!window.supabase){button.disabled=true;status.textContent='Password recovery is temporarily unavailable.';return;}const supabase=window.supabase.createClient(config.url,config.key,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});button.addEventListener('click',async()=>{const email=emailInput.value.trim();if(!email||!emailInput.checkValidity()){status.textContent='Enter the account email first.';emailInput.focus();return;}button.disabled=true;status.textContent='Requesting a secure reset link...';const{error}=await supabase.auth.resetPasswordForEmail(email,{redirectTo:\`\${window.location.origin}/auth/update-password\`});if(error){status.textContent='Unable to request a reset link. Please try again.';button.disabled=false;return;}status.textContent='If the account is eligible, a reset link has been sent.';});})();</script></body></html>`;
  return { html, nonce };
}

function recoveryPage() {
  const { nonce, config } = authPageContext();
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Update password · SC Agent OS</title><style>${authStyles()}</style></head><body><main class="box"><div class="brand">SC / DCSE PASSWORD RECOVERY</div><div class="sub">Choose a new password for the authorized account.</div><form id="passwordForm"><label for="newPassword">New Password</label><input class="field" id="newPassword" type="password" autocomplete="new-password" minlength="12" maxlength="72" required disabled><label for="confirmPassword">Confirm Password</label><input class="field" id="confirmPassword" type="password" autocomplete="new-password" minlength="12" maxlength="72" required disabled><button class="btn" id="updatePassword" type="submit" disabled>Update Password</button></form><div class="message" id="recoveryStatus" role="status" aria-live="polite">Validating the recovery link...</div><div class="hint">For safety, recovery links are single-purpose and may expire. Tokens are never displayed or logged.</div><a class="btn secondary" href="/">Return to sign in</a></main><script src="${SUPABASE_BROWSER_SRC}" crossorigin="anonymous"></script><script nonce="${nonce}">(() => {const config=${config};const form=document.getElementById('passwordForm');const passwordInput=document.getElementById('newPassword');const confirmInput=document.getElementById('confirmPassword');const button=document.getElementById('updatePassword');const status=document.getElementById('recoveryStatus');let ready=false;let timer;function showInvalid(){if(ready)return;status.textContent='This recovery link is invalid or expired. Request a new link from the sign-in page.';status.className='message err';}function enableRecovery(){if(ready)return;ready=true;clearTimeout(timer);history.replaceState(null, '', '/auth/update-password');passwordInput.disabled=false;confirmInput.disabled=false;button.disabled=false;status.textContent='Recovery link verified. Enter your new password.';status.className='message ok';passwordInput.focus();}const query=new URLSearchParams(window.location.search);const hash=new URLSearchParams(window.location.hash.slice(1));if(query.has('error')||hash.has('error')){showInvalid();return;}if(!config.url||!config.key||!window.supabase){status.textContent='Password recovery is temporarily unavailable.';status.className='message err';return;}const supabase=window.supabase.createClient(config.url,config.key,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});const hintedRecovery=hash.get('type')==='recovery'||query.get('type')==='recovery';supabase.auth.onAuthStateChange((event,session)=>{if(event === 'PASSWORD_RECOVERY'&&session)enableRecovery();});supabase.auth.getSession().then(({data,error})=>{if(!error&&hintedRecovery&&data.session)enableRecovery();});timer=setTimeout(showInvalid,4000);form.addEventListener('submit',async event=>{event.preventDefault();if(!ready)return showInvalid();const newPassword=passwordInput.value;const confirmation=confirmInput.value;if(newPassword.length<12||newPassword.length>72||!/[A-Za-z]/.test(newPassword)||!/[0-9]/.test(newPassword)){status.textContent='Use 12–72 characters with at least one letter and one number.';status.className='message err';return;}if(newPassword!==confirmation){status.textContent='The passwords do not match.';status.className='message err';return;}button.disabled=true;status.textContent='Updating password...';status.className='message';const{error}=await supabase.auth.updateUser({ password: newPassword });if(error){status.textContent='Password update failed. The link may have expired; request a new one.';status.className='message err';button.disabled=false;return;}status.textContent='Password updated. Returning to sign in...';status.className='message ok';await supabase.auth.signOut({scope:'local'});setTimeout(()=>window.location.replace('/?password_updated=1'),1200);});})();</script></body></html>`;
  return { html, nonce };
}
