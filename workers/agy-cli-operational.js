...
function runAgyToolFree(expected) {
...
function parseResponse(wrapper) {
  ...
  try { return JSON.parse(response); } catch { throw new Error(`AGY_RESPONSE_NOT_JSON:${response.slice(0,500)}`); }
}

async function main() {
  await obtainToken(true);
  const who = await rpc('v7_worker_whoami');
  console.log(JSON.stringify({event:'cert_worker_started', agent_id:cfg.agentId, session_id:sessionId, expected_task_id:cfg.expectedTaskId, identity:who}));
  ...
  try {
    ...
    const actual = parseResponse(run.wrapper);
    if (JSON.stringify(actual) !== JSON.stringify(expected)) throw new Error('AGY_RESPONSE_MISMATCH');

    const output = {
      contract_version:'dcse-inter-agent-v1', task_id:task.task_id, claim_id:task.claim_id,
      agent_id:cfg.agentId, session_id:sessionId, runtime_surface:'agy_windows_cli', provider:'antigravity',
      started_at:run.started_at, completed_at:run.completed_at, status:'completed', raw_exit_code:run.exit_code,
      timed_out:false, deterministic_snapshot:snapshot, provider_output:actual,
      provider_wrapper_status:run.wrapper.status || null, provider_denied_actions:run.wrapper.denied_actions || [], stderr:run.stderr,
    };

    await rpc('v7_worker_submit_result', {p_claim_id:Number(task.claim_id), p_event_type:'worker_completed', p_output:output, p_worker_session_id:sessionId});
