...
def _http_json(url: str, *, method: str = "GET", headers: dict[str, str] | None = None,
               payload: Any = None, timeout: int = 30) -> Any:
    ...
    try:
        ...
        raise WorkerError(f"connection_failed:{exc.reason}") from None
...
def _process_orchestrate(turn: dict[str, Any]) -> None:
    ...
    interpretation_text, raw_interpret = _ollama([
        {
            ...
            "content": (
                ...
                "Return valid JSON only with key operation_candidate."
            ),
        },
    ...
    try:
        ...
        candidate = interpreted.get("operation_candidate") or {}
    except Exception as exc:
        raise WorkerError(f"operation_candidate_invalid_json:{exc}") from None
    if not isinstance(candidate, dict) or not candidate.get("objective"):
        raise WorkerError("operation_candidate_missing_required_fields")
    ...
    try:
        ...
        raise WorkerError(f"response_invalid_json:{exc}") from None
    if not content:
        raise WorkerError("response_content_empty")
