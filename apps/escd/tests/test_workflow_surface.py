...
class WorkflowSurfaceTests(unittest.TestCase):
    ...
    def test_operator_workflow_ui_is_not_backend_only(self):
        for required in [
            ...
            'id="workflowDetail"',
            'id="workflowTemplate"',
            'id="workflowContextType"',
            'id="workflowContextRef"',
            'id="workflowBindings"',
            'id="workflowSourceRefs"',
            'id="workflowJobId"',
            'id="instantiateWorkflow"',
            'Template', 'Version', 'Execution', 'Steps',
            'Approval:', 'Evidence:', 'Attempts:', 'Next permitted step:',
        ]:
    ...
    def test_candidate_entrypoint_and_rewrite_are_specific_before_generic(self):
    ...
    def test_no_ddna_or_employment_scope_was_bundled_into_workflow_surface(self):
        combined = (self.workflow_api + self.workflow_handler + self.entrypoint + self.html).lower()
