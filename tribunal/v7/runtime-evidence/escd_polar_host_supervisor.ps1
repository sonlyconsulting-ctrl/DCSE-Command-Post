...
function Invoke-EscdPolarHostSupervisor {
  ...
  $hostCount = ($hostRows | Measure-Object).Count
  if ($hostCount -eq 0) {
    $result.action = 'HOLD'
  ...
  if ($pendingCount -gt 0) {
    try { $result.turn_key = $pending[0].turn_key } catch {}
  }

  if ($Mode -eq 'ON_DEMAND' -and $pendingCount -eq 0) {
    $result.action = 'IDLE'
