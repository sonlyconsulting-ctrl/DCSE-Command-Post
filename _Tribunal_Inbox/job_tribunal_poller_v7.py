...
def terminal_receipt_exists(path: Path, source_sha256: str, dispatch: bool = True) -> bool:
    if not path.is_file():
        return False
    try:
        data = json.loads(path.read_text(encoding="utf-8-sig"))
        if str(data.get("source_sha256", "")).upper() != source_sha256.upper():
            return False
        ...
        if state in {
            ...
            return True
        if not dispatch and state == PollerState.AUTHORIZED.value and outcome == "AUTHORIZED_DRY_RUN_HOLD":
            return True
        return False
...
def process_packet(
    ...
    try:
        ...
        return {"outcome": "SKIPPED_LOCKED_FILE", "error": str(exc), "source_file": str(path)}
    ...
    if terminal_receipt_exists(out, source_sha256, dispatch):
    ...
    try:
        ...
        current_state = PollerState(receipt["state"])
        if current_state in {PollerState.AUTHORIZED, PollerState.DISPATCHED}:
            transition(receipt, PollerState.FAILED, str(exc))
            receipt["outcome"] = "FAILED_GOVERNANCE_ERROR"
        else:
