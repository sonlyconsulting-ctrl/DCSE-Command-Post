# CTJ Final Pricing Disposition Recommendation

Task ID: SC-CTJ-COMMERCIAL-PACKAGING-PAYMENTS-20260912-16
Lane: SC / CTJ
Date: 2026-09-12
Status: FINAL RECOMMENDATION FOR DCS LEVEL 0 LOCK
Gate owner: DCS Level 0

## Final recommended launch ladder

| Product | Price |
| --- | ---: |
| Strategic Clarity Assessment | $20 |
| Focus & Flow | $20 |
| Mental Ingenuity | $20 |
| Focus & Flow + Mental Ingenuity | $30 |
| SCA + Focus & Flow + Mental Ingenuity | $45 first-time promo |
| Part 1 | $39 |
| Part 2 | $39 |
| Part 3 | $39 |
| Parts 1-3 Collection | $99 |
| Unified Edition | $119 |
| Complete CTJ Keeper Collection | $199 |

Existing SC membership plans:
NO CHANGE.

## Market-anchor validation

Mainstream premium individual AI subscriptions currently cluster at approximately $20/month:
- ChatGPT Plus: $20/month
- Claude Pro: $20/month
- Google AI Pro: $19.99/month
- Perplexity Pro: $20/month

This supports the $20 price point for each smaller standalone CTJ product.

The comparison is a reference point only. CTJ products are not model-access subscriptions.

## Value ladder

$20:
paid entry products: Discover, Practice, or Prove

$45:
first-time Discover + Practice + Prove promotional trio

$30:
Practice + Prove pair; $10 / 25% savings versus two $20 purchases

$39:
full structured learning module

$99:
Parts 1-3 learning collection; $18 / 15.4% savings versus three $39 purchases

$119:
premium synthesis capstone

$199:
complete owned CTJ suite

Cheapest evergreen equivalent bundle path outside Complete Collection:
- SCA: $20
- Focus & Flow + Mental Ingenuity: $30
- Parts 1-3 Collection: $99
- Unified: $119
- total: $268

Complete Collection at $199:
- $69 lower
- approximately 25.7% suite discount

First-time promotional path:
- SCA + Focus & Flow + Mental Ingenuity: $45
- Parts 1-3 Collection: $99
- Unified: $119
- total: $263

Complete Collection remains $64 lower than that promotional path.

## Future open-source AI principle

DCS intends to develop/use an open-source model path for future AI capability.

Commercial rule:
Do not reduce base product prices merely because local/open-source inference can lower marginal model cost.

Price the owned CTJ products on customer value and product scope.

Keep future variable-cost AI capability separate from the owned base product where appropriate:
- member-only advanced AI
- optional AI enhancement
- usage-governed AI tier
- local/open-source inference when economically and technically appropriate

Owned base products should continue to function without requiring a paid frontier-model subscription unless DCS explicitly changes that architecture.

## Repricing triggers

Do not repeatedly reprice based on minor competitor changes.

Reopen pricing only if one of the following becomes material:
1. actual production unit economics materially weaken margin
2. payment/support/hosting/AI variable cost materially changes
3. product scope materially expands or contracts
4. CTJ market positioning materially changes
5. mainstream premium-AI reference pricing shifts enough to change customer expectations
6. DCS intentionally changes membership/Keeper economics

## Final recommendation

Lock this ladder for the initial CTJ commercial build.

No further broad pricing review is recommended before:
- product-card implementation
- payment/verification workflow implementation
- catalog reconciliation
- production cost reconciliation

This record is a recommendation and does not self-create DCS Level 0 approval.


## Promotional-entry decision

The full SCA is not free.

A free mini clarity ad/lead asset may be created separately, but it does not yet exist as a verified product.

Approved candidate promotional architecture:
- SCA: $20
- Focus & Flow: $20
- Mental Ingenuity: $20
- Focus & Flow + Mental Ingenuity: $30 evergreen pair
- SCA + Focus & Flow + Mental Ingenuity: $45 first-time introductory promotion

The Intro Trio presents three different CTJ modes at once:
Discover + Practice + Prove.

Do not market the offer as a clearance sale.
