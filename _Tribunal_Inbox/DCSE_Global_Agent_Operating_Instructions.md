...
## 4. Tribunal Mandatory Reporting Rule (Adopted 2026-06-18)
To maintain the tribunal-style communication layer as an active shared operating record across sessions, every agent session must end with a structured report addressing the following eight points:
1. **Files read:** [List all files read during the session]
2. **Files created:** [List all files created during the session]
3. **Files edited:** [List all files edited during the session]
4. **Files skipped:** [List any files that were skipped and why]
5. **Restrictions followed:** [Confirm compliance with firewalls, local-only limits, non-destructive rules, etc.]
6. **Pending DCS response items:** [Highlight any open questions or approvals requested from DCS]
7. **Next recommended action:** [Propose the logical next step for the next agent/session]
8. **JSON updated and validated:** [Confirm update and validation of TRIBUNAL_*.json and/or tribunal_activity.json]
